 ...
 * @since 2.2
 */
public class ReversedLinesFileReaderLineSeparator implements Closeable {
     ...
     */
    public ReversedLinesFileReaderLineSeparator(final File file) throws IOException {
     ...
     * @since 2.3
     */
    public ReversedLinesFileReaderLineSeparator(final File file, final int blockSize, final Charset encoding) throws IOException {
     ...
     *             supported.
     */
    public ReversedLinesFileReaderLineSeparator(final File file, final int blockSize, final String encoding) throws IOException {
    ...
    private class FilePart {
        ...
        private String readLine() throws IOException {
            ...
            while (i > -1) {
                ...
                // --- check for newline ---
                LineEndData lineEndData = getNewLineMatchByteCount(data, i);
                if ((newLineMatchByteCount = lineEndData.length) > 0 /* found newline */) {
                    ...
                    if(lineData.length > 0) {
                        line = new String(lineData, encoding) + new String(lineEndData.sequence);
                    } else {
         ...
         */
        private LineEndData getNewLineMatchByteCount(byte[] data, int i) {
            for (byte[] newLineSequence : newLineSequences) {
                ...
                if (match) {
                    return new LineEndData(newLineSequence);
                }
            }
            return new LineEndData(null);
        }
        
        private class LineEndData {
            int length;
            byte[] sequence;
            
            public LineEndData(byte[] sequence) {
                if(sequence != null) {
